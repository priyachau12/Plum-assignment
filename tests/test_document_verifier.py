"""Tests for the deterministic document verification (TC001–TC003 + a clean case)."""

from __future__ import annotations

from app.models.decision import BlockingReason
from app.verification.document_verifier import verify_documents
from tests.helpers import claim_request


def test_tc001_missing_required_document_names_both_types(policy):
    result = verify_documents(claim_request("TC001"), policy)
...
def test_tc002_unreadable_document_asks_for_reupload_without_rejecting(policy):
    result = verify_documents(claim_request("TC002"), policy)
    assert not result.passed
    issue = next(
...
def test_tc003_patient_mismatch_names_both_patients(policy):
    result = verify_documents(claim_request("TC003"), policy)
    assert not result.passed
...
def test_tc004_clean_claim_passes_but_still_traces_every_check(policy):
    result = verify_documents(claim_request("TC004"), policy)
    assert result.passed
    assert result.blocking_issues == []
    steps = {entry.step for entry in result.trace_entries}
    assert steps == {
        "verify.required_documents",
        "verify.readability",
        "verify.patient_consistency",
    }
