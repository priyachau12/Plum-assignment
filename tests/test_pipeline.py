...
def test_clean_claim_runs_full_pipeline(policy):
    ...
    assert "verify.required_documents" in steps
    assert "adjudicate.financials" in steps
    assert "explain" in steps
...
def test_blocked_claim_skips_decision(policy):
    ...
    assert not any(s.startswith("adjudicate") for s in steps)
    assert "explain" not in steps
