"""check_documents node — the early-stop gate.

Thin adapter over the pure `run_document_checks` logic. It runs the checks,
...
- Calls `verification.document_checks.run_document_checks`.
...
def check_documents(state: ClaimState, *, policy: Policy) -> dict:
    request = state["request"]
    result = run_document_checks(request, policy)

    logger.info(
        "check_documents: passed=%s blocking_issues=%d",
        result.passed,
